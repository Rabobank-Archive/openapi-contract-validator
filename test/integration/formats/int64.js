...
describe(`The format property: int64`, function() {
    beforeEach(function() {
        this.options = {
            statusCode: 200,
            endpoint: '/endpoint',
    ...
    it('throws an error when the value is not a valid int64', function(done) {
        ...
        validator.validate(this.contract, response, this.options)
            .then(() => {
                ...
                expect(c.unstyle(error.message))
                    .to.have.string('MAXIMUM should be <= 9223372036854776000');
            })
            .then(done, done);
    });

    it('passes when it the value is a valid int64', function(done) {
        const response = newResponse({
            foo: 123,
        });
